export class LevelRenderer {
    ...
    async loadAssets(tilesFolder = "tiles", bgPath = "bg.svg") {
        try {
            ...
            this.needsHue = Array.from({ length: 86 }, (_, i) => i).filter(i => i % 5 === 0);
    ...
    applyColorEffect(source, hueShift) {
        try {
            ...
            for (let i = 0; i < data.length; i += 4) {
                ...
                const [r, g, b] = this.hsvToRgb((h + hueShift) % 360, s, v);
    ...
    getTileWithEffects(tileIndex, hue, rotation) {
        try {
            ...
            if (hue !== '0' && this.needsHue.includes(tileIndex % 86)) {
                tile = this.applyColorEffect(tile, parseInt(hue));
            }
    ...
    async renderLevelImage(levelData) {
        try {
            ...
            const hue = levelData.hue === 'Infinity' ? 0 : parseFloat(levelData.hue);
            const hue2 = levelData.hue2 === 'Infinity' ? 0 : parseFloat(levelData.hue2);
